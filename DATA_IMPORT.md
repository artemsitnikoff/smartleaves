...
### Вариант А: Через Git (рекомендуется для JSON и скриптов)
...
ls -lh *_data.json import_*.py
```

### Вариант Б: Через scp (только для медиа файлов)

```bash
# На локальной машине:
# Скопируйте только архив с медиа файлами (50 МБ)
scp media_files.tar.gz deploy@37.9.5.160:/var/www/smartleaves/
```

**Рекомендация:** Используйте git для JSON и скриптов (уже в репозитории), scp только для `media_files.tar.gz`

## Шаг 2: Импорт на сервере

### Вариант А: Автоматический импорт (рекомендуется)

Один скрипт импортирует все данные:
...
2. Импортирует категории, теги, медиа файлы, worksheets
3. Покажет итоговую статистику

### Вариант Б: Ручной импорт (пошагово)
...
# Подтяните последние изменения с данными
git pull origin main
...
**Примечание:** JSON файлы и скрипты уже на сервере через git, просто копируем их в контейнер.
